...
FILE_ROOT = "archive/"
IMAGE_SET_ROOT = FILE_ROOT + "VOC2021_suitcase/ImageSets/Main"  # 图片区分文件的路径
IMAGE_PATH = FILE_ROOT + "VOC2021_suitcase/JPEGImages"  # 图片的位置
ANNOTATIONS_PATH = FILE_ROOT + "VOC2021_suitcase/Annotations"  # 数据集标签文件的位置
LABELS_ROOT = FILE_ROOT + "VOC2021_suitcase/Labels"  # 进行归一化之后的标签位置
DEST_PPP = FILE_ROOT + "suitcase_yolo_format"
DEST_IMAGES_PATH = "suitcase_yolo_format/images"  # 区分训练集、测试集、验证集的图片目标路径
DEST_LABELS_PATH = "suitcase_yolo_format/labels"  # 区分训练集、测试集、验证集的标签文件目标路径
...
def get_xml_data(file_path, img_xml_file):
    ...
    img_jpg_file_name = IMAGE_PATH + '/' + img_xml_file + '.jpg'
    # print(img_jpg_file_name)
    cv2.imread(img_jpg_file_name)
    img = cv2.imread(img_jpg_file_name)
    ...
    if img_size is not None:
        ...
        # if len(img_size.getElementsByTagName("depth")) > 0:
        #     img_c = int(img_size.getElementsByTagName("depth")[0].childNodes[0].data)
    else:
        img_h, img_w, img_c = cv2.imread(img_jpg_file_name).shape
